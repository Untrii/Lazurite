...
function parseOptions(options: string) {
  ...
  correctKeys.push('shift', 'ctrl', 'alt', 'delete')
...
export default function useHotkey(options: IHotkeyOptions | string, callback: Function) {
  ...
  const keys = options.keys.map((key) => key.toUpperCase())
  let codes = []

  for (const key of keys) {
    if (key.length == 1 && key >= 'A' && key <= 'Z') codes.push('Key' + key)
    if (key.length == 1 && key >= '0' && key <= '9') codes.push('Digit' + key, 'Numpad' + key)
