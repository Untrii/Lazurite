...
export default class HistoryService {
  ...
  registerSlideCreate() {
    this.registerAction('createSlide', { index: presentation.slides.length - 1 }, null, null)
  }
  ...
  registerCut(elements: ISlideObject[], slideIndex: number) {
    const ids: string[] = []
    for (const item of elements) {
      ids.push(item.id)
    }
    this.registerAction('cut', { id: ids.join(';'), index: slideIndex }, elements, null)
  }

  registerPaste(elements: ISlideObject[], slideIndex: number) {
    const ids: string[] = []
    for (const item of elements) {
      ids.push(item.id)
    }
    this.registerAction('paste', { id: ids.join(';'), index: slideIndex }, elements, null)
  }
  ...
  async registerAction(actionType: string, waybackArgs: IWaybackProperties, oldValue: any, newValue: any) {
    ...
    switch (actionType) {
      ...
      case 'createElement':
        ...
        break
      case 'paste':
        newRecord.waybackFunction = 'createMultipleElementsWayback'
        break
    }
  ...
  createSlideWayback(record: IHistoryRecord, reverse?: boolean) {
    if (!reverse) {
      if (typeof record.waybackArguments.index == 'number') slideService.deleteSlide(record.waybackArguments.index)
    } else {
  ...
  createMultipleElementsWayback(record: IHistoryRecord, reverse?: boolean) {
    const slide = presentation.slides[record.waybackArguments.index ?? 0]
    if (!reverse) {
      for (const item of record.oldValue) {
        try {
