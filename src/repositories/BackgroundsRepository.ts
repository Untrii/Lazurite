...
export default class BackgroundsRepository extends SingletoneRepository implements IBackgroundCollection {
  static init(filePath: string) {
    const model = getBlankCollection()
  ...
  static get Instance(): BackgroundsRepository {
    console.log('here')
    return this.getInstanse(() => this.init('data/bg.json'))
  }
