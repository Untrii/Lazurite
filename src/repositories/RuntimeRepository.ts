...
const defaultModel: RuntimeData = {
...
export default class RuntimeRepository extends SingletoneRepository {
  static get Instance(): RuntimeRepository {
    return this.getInstanse(() => {
      const verificator = ModelVerificator.createVerificator(defaultModel)
      const fileHandle = ReactiveFileHandle.create('runtime', verificator, defaultModel, new MemoryFileSystem())
