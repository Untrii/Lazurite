...
export default class DraggableResizable extends Vue {
  ...
  computeStickStyle(stick) {
    const stickSize = realStickSize / this.parentScale
    const stickStyle: any = {
      width: `${stickSize}px`,
      height: `${stickSize}px`,
    }
    const offset = -stickSize / 2 + 'px'
  ...
  applyAspectRatio(rect) {
    ...
    const stick = this.pressedStick
    const startRect = this.startRect
    const startRatio = startRect.width / startRect.height
    const width = rect.height * startRatio
    const height = rect.width / startRatio
  ...
  applySnapToGrid(rect) {
    ...
    if (this.lastEvent == 'bodyDown') {
      //snap x
      const left0 = Math.round(rect.left / gridX) * gridX
      const left1 = Math.round((rect.left + rect.width) / gridX) * gridX - rect.width

      if (Math.abs(rect.left - left0) < Math.abs(rect.left - left1)) rect.left = left0
      else rect.left = left1

      //snap y
      const top0 = Math.round(rect.top / gridY) * gridY
      const top1 = Math.round((rect.top + rect.height) / gridY) * gridY - rect.height

      if (Math.abs(rect.top - top0) < Math.abs(rect.top - top1)) rect.top = top0
      else rect.top = top1
    }
    if (this.lastEvent == 'stickDown') {
      const ntop = Math.round(rect.top / gridY) * gridY
      const nleft = Math.round(rect.left / gridY) * gridY
