...
export default class LzRangeInput extends Vue {
  ...
  get flooredValue() {
    const valParts = this.localValue.toString().split('.')
    const intPart = valParts[0]
    const fractPart = valParts[1] ?? ''
    const fractCharArr: string[] = []
  ...
  onMouseMove(event: MouseEvent) {
    ...
    const range = this.$el.children[1]
    const width = (range.clientWidth - 45) * 0.95 //minus reset button
    const offset = event.pageX - range.getBoundingClientRect().x - width / 19
    const progress = this.min + (offset / width) * (this.max - this.min)
